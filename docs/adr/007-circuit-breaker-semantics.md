...
- 增补（2026-09-05，随 ADR-010 同款缺陷修正）：降级期的恢复判定只认被指派的顺路探针——健康期发出、降级后才返回的迟到成功只返回自己的裁决，不改状态。此前“降级期任一成功即恢复”会让故障瞬间在途的响应把状态翻回健康再降级，每翻一次备胎切换一次；缓存存取（ADR-009 决策 4 的同款粘滞规则）同步修正。
