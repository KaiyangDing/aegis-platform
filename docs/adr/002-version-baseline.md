...
- **langchain 1.4.0** 加入基线（`uv add langchain`）：M2 起 `create_agent` 与 `AgentMiddleware` 的来源。解析未改动任何既有锁定版本（langchain-core 1.6.1 / langgraph 1.2.11 / langgraph-checkpoint 4.2.0 / langgraph-checkpoint-postgres 3.1.2 不变）。M2 的全部框架事实以本仓 1.4.0 探针为准，结论写入 ADR-011 起各 ADR 的实证节。
- **psycopg-pool 3.3.1** 由传递依赖显式化（`uv add psycopg-pool`）：checkpointer 的连接池由本仓生产代码直接 import，与 M1 显式化 openai / httpx2 同一纪律。
